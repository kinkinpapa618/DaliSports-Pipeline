# upload_yt_nopublish_caulong.py - Upload video cau long len YouTube, dien title/chapters
# KHONG tu bam phan tich/publish. Giu cua so mo de nguoi dung tu xuong ban.
...
LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "upload_yt_caulong_log.txt")

VIDEO_FILE = "caulong_ttbc.mp4"
NORMS_FILE = "caulong_ttbc_timeline_norms.txt"
...
async def main():
    ...
    if not os.path.exists(video_path):
        print(f"[!] Khong tim thay video: {video_path}"); return
    if not os.path.exists(norms_path):
        print(f"[!] Khong tim thay norms: {norms_path}"); return

    title, description = u.build_youtube_chapters_and_title(video_path, norms_path)
    print("=" * 60, flush=True)
    print("UPLOAD YOUTUBE - CAU LONG (KHONG TU PUBLISH)", flush=True)
    ...
    async with async_playwright() as p:
        ...
        if "accounts.google" in page.url:
            print(">>> VUI LONG DANG NHAP YOUTUBE TRONG TRINH DUYET...", flush=True)
        ...
        print("  [>] Dang chon tep video...", flush=True)
        ...
        print("  [+] Da chon tep, cho tai len hoan tat (4.41GB, co the lau)...", flush=True)
        ...
        while time.time() < upload_deadline:
            try:
                ...
                clean_streak = 0
            await asyncio.sleep(6)
        print("  [OK] Upload hoan tat - BAT DAU DIEN THONG TIN" if upload_done else "  [!] Het thoi gian cho upload", flush=True)
        ...
        try:
            ...
            print("  [+] Da dien mo ta kem Chapters", flush=True); await asyncio.sleep(1)
        except Exception as e:
        ...
        print("  CUA SO DANG DUOC GIU MO - BAN TU BAM:", flush=True)
        print("     1) Nut 'Tiep theo' (Next) nhieu lan qua cac buoc", flush=True)
        print("     2) Cuoi cung chon che do + bam 'Xuat ban'", flush=True)
        print("  Script se doi 2 tieng (giu cua so). Muon tat: dong cua so.", flush=True)
        print("=" * 60, flush=True)

        while True:
            try:
                if page.is_closed():
                    print("[*] Trinh duyet da dong, ket thuc.", flush=True)
                    break
            except Exception:
                break
